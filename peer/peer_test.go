	...
	tests := []struct {
	...
	}{
		{
			...
			func() (*peer.Peer, *peer.Peer, error) {
				inConn, outConn := pipe(
					&conn{raddr: "10.0.0.1:3888"},
					&conn{raddr: "10.0.0.2:3888"},
				)
				inPeer := peer.NewInboundPeer(peer1Cfg)
				inPeer.AssociateConnection(inConn)

				outPeer, err := peer.NewOutboundPeer(peer2Cfg, "10.0.0.2:3888")
				if err != nil {
					return nil, nil, err
				}
		...
		},
		{
			...
			func() (*peer.Peer, *peer.Peer, error) {
				inConn, outConn := pipe(
					&conn{raddr: "10.0.0.1:3888", proxy: true},
					&conn{raddr: "10.0.0.2:3888"},
				)
				inPeer := peer.NewInboundPeer(peer1Cfg)
				inPeer.AssociateConnection(inConn)
...
func TestPeerListeners(t *testing.T) {
	...
	outPeer, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:3888")
	if err != nil {
...
func TestOutboundPeer(t *testing.T) {
	...
	r, w := io.Pipe()
	c := &conn{raddr: "10.0.0.1:3888", Writer: w, Reader: r}

	p, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:3888")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
	...
	r1, w1 := io.Pipe()
	c1 := &conn{raddr: "10.0.0.1:3888", Writer: w1, Reader: r1}
	p1, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:3888")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
	...
	r2, w2 := io.Pipe()
	c2 := &conn{raddr: "10.0.0.1:3888", Writer: w2, Reader: r2}
	p2, err := peer.NewOutboundPeer(peerCfg, "10.0.0.1:3888")
	if err != nil {
		t.Errorf("NewOutboundPeer: unexpected err - %v\n", err)
		return
	}
...
func TestUnsupportedVersionPeer(t *testing.T) {
	...
	localConn, remoteConn := pipe(
		&conn{laddr: "10.0.0.1:3888", raddr: "10.0.0.2:3888"},
		&conn{laddr: "10.0.0.2:3888", raddr: "10.0.0.1:3888"},
	)
